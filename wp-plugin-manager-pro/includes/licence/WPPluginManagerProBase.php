<?php
if(!class_exists("WPPluginManagerProBase")) {
	class WPPluginManagerProBase {
    	public $key = "1379C963B8C52EB3";
    	private $product_id = "8";
    	private $product_base = "wp-plugin-manager-pro";
		...
		function __construct($plugin_base_file='')
		{
			...
			if($this->hasCheckUpdate) {
				if(function_exists("add_action")){
					add_action( 'admin_post_wp-plugin-manager-pro_fupc', function(){
					...
					if($this->isTheme){
						...
						add_filter( 'plugin_row_meta', function($links, $plugin_file ){
							if ( $plugin_file == plugin_basename( $this->pluginFile ) ) {
								$links[] = " <a class='edit coption' href='" . esc_url( admin_url( 'admin-post.php' ) . '?action=wp-plugin-manager-pro_fupc' ) . "'>Update Check</a>";
							}
							return $links;
						}, 10, 2 );
